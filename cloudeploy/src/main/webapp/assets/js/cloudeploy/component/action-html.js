/**
 * to paint actionPanel
 */
var actionPanelHtml = {
	/**
	 * paint ComponentTypes list
	...
	/**
	 * paint componet list and attach it to componnt types
	 * @param components
	 * @param show
	 */
	...
	paintNode : function(component, nodeId, actionName) {
		...
		var html = '<div id='
				...
				+ component.id
				+ '" class="graph-node">'
				+ '<div class="node-title"><i class="fa fa-minus-circle node-del-btn" onclick="javascript:actionPanel.deleteNode('
				+ id + ')"></i>' + component.displayName + '</div>'
				+ '<div class="node-oper">' + actionName + '</div>' + '</div>';
		$("#tmp-panel").append(html);
		$("#" + id).unbind("dblclick");
		$("#" + id).dblclick(function() {
			actionPanel.nodeClick(id);
		});
		actionPanelHtml.paintEndPoint(id);
		return id;
	},
	...
	paintNodeDetailAction : function(node) {
		...
		actionPanelHtml.paintActionParams(node.action.params, node.params);
	},
		...
		if (tasks.length > 0) {
			for ( var i in tasks) {
				...
				html += '<tr><td style="display:none;">'
						...
						+ '</td><td>'
						+ '<i class="fa fa-edit text-success small"></i> <a class="link-btn" href="javascript:actionPanel.changeView(2,{taskId:'
						+ task.id
						+ '});">编辑</a>'
						+ '<i class="fa fa-file-code-o text-success small"></i> <a class="link-btn" target="_blank" href="/cloudeploy/views/task/source?id='
						+ task.xmlFile
						+ '">查看源文件</a>'
						+ '<i class="fa fa-remove text-danger small"></i> <a class="link-btn" href="javascript:actionPanel.deleteTask('
						+ task.id + ');">删除</a>' + '</td></tr>';
			}
		} else {
	...
	paintExecList : function(execs, execContent) {
		...
		if (verifyParam(execContent)) {
			actionPanelHtml.paintExecContent(execContent);
		}

		execControl.unbind("change");
		execControl.bind('change', function() {
			var exec = actionPanel.cachedExecs.get($(this).val());
			actionPanelHtml.paintExecContent(exec.content);
		});

		$("#execContent").unbind("change");
		$("#execContent").bind('change', function() {
			actionPanel.execChanged();
		});
