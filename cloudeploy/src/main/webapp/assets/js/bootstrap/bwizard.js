...
(function ($, undefined) {
    $.widget("bootstrap.bwizard", {
        ...
        _refreshStep: function () {
            ...
            if (this.lis) {
                this.lis.removeClass('active').attr('aria-selected', false).find('.label').removeClass('badge-success');
                if (o.activeIndex >= 0 && o.activeIndex <= this.lis.length - 1) {
                    if (this.lis) {
                        this.lis.eq(o.activeIndex).addClass('active').attr('aria-selected', true).find('.label').addClass('badge-success');
                    }
                }
            }
